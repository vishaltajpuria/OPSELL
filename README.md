...
- `lib/runDailyStrategy.ts` holds the actual run logic (shared by the cron route and the manual `/api/strategy/run` endpoint behind the Strategy tab's button), rate-limited to Kite's 3 req/sec historical-data cap in batches of 3, scanning the top `TOP_N_STOCKS_BY_LIQUIDITY` (120) F&O stocks by liquidity score rather than the full universe, to stay under Vercel Hobby's 60s cap. It checkpoint-saves after the stocks phase so an indices-phase timeout doesn't lose the run. `maxDuration` is set to 300s, but Vercel's Hobby plan hard-caps actual function duration at 60s regardless; upgrading to Pro removes that cap (and `TOP_N_STOCKS_BY_LIQUIDITY` could then be raised or dropped).
