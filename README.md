...
**How the daily Strategy signals work:** every trading day at 4:00 PM IST, a scheduled job fetches historical price data and checks, per stock/index: is Supertrend(14,1) in an uptrend with price above its line, and did the SMA20 just cross from at/below the Supertrend line to above it? That's a **short** signal, with a target of the nearest SMA (50/100/200) below price. The mirror image — Supertrend downtrend, SMA20 crossing from at/above the line to below it — is a **long** signal. Stocks run on the Daily timeframe (Liquid bucket only); the five indices run on both Daily and 4H. The app only surfaces short/long calls — turning a signal into an actual option trade (strike, expiry, buy vs. sell) is left to you.
